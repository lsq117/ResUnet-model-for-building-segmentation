...
def create_conv_net(x, keep_prob, channels, n_class, layers=3, features_root=16, filter_size=3, pool_size=2,
    ...
    for layer in range(0, layers):
        with tf.name_scope("down_conv_{}".format(str(layer))):
            ...
            dw_h_convs[layer] = tf.nn.relu(tf.layers.batch_normalization(conv2))
    ...
    for layer in range(layers - 2, -1, -1):
        with tf.name_scope("up_conv_{}".format(str(layer))):
            ...
            in_node = tf.nn.relu(tf.layers.batch_normalization(conv2))
...
class Unet(object):
    ...
    def __init__(self, channels, n_class, cost="dice_coefficient", cost_kwargs={}, **kwargs):
        ...
        #self.is_training = tf.placeholder(tf.bool, name="batch_normalization")
...
class Trainer(object):
    ...
    def train(self, data_provider, output_path, training_iters=10, epochs=100, dropout=0.75, display_step=1,
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(epochs):
                ...
                self.store_prediction(sess, batch_x, batch_y, "epoch_%s" % epoch)
    ...
    def store_prediction(self, sess, batch_x, batch_y, name):
        ...
        #logging.info("pred_shape={:}".format(pred_shape))

        loss = sess.run(self.net.cost, feed_dict={self.net.x: batch_x,
                                                  self.net.y: batch_y,
                                                  self.net.keep_prob:1.})

        logging.info("Verification error= {:.1f}%, loss= {:.4f},pred_shape={:}".format(error_rate(prediction,
                                                                                   batch_y),
                                                                        loss,prediction.shape))
